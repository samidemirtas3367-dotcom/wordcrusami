...
<script>
    ...
    */
    ...
    function buildGrids() {
        ...
        const pool = MASTER_DICT[GAME_STATE.unit].words;

        for(let i=0; i<15; i++) {
            const word = pool[Math.floor(Math.random()*pool.length)];
            const color = Math.floor(Math.random()*4);
            enG.appendChild(createTile(word.en, 'en', i, word.en, color));
            trG.appendChild(createTile(word.tr, 'tr', i, word.en, 'tr'));
        }
    ...
    function createTile(val, type, idx, pair, col) {
        ...
        // Special class logic for TR tiles to avoid accidental coloration
    ...
    function checkPair() {
        ...
        if(en.dataset.pair === tr.dataset.pair && en.dataset.pair !== "") {
            ...
            updateUI();
            // MANUAL MATCH: SADECE TEMİZLE (YENİ KELİME GELMEZ)
            removeTiles([en, tr], false);
        } else {
    ...
    function scanForMatch3() {
        if(GAME_STATE.bonusActive) return; // Prevent cascading loops if unnecessary

        const tiles = Array.from(document.querySelectorAll('#en-grid .tile'));
        let matchSet = new Set();

        // Horizontal Scan (r=rows)
        for(let r=0; r<5; r++) {
        ...
        if(matchSet.size > 0) {
            GAME_STATE.score += 500; // Large bonus
            flashBonus();
            updateUI();
            GAME_STATE.bonusActive = true;
            // MATCH-3 SPECIAL: REGENERATE TILES (shouldRegen = true)
    ...
    function removeTiles(list, shouldRegen) {
        list.forEach(t => {
            const pairTR = document.querySelector(`#tr-grid .tile[data-index="${t.dataset.index}"]`);
            
            // FIRE POP ANIMATION if shouldRegen
            t.className += (shouldRegen) ? ' fire-pop' : ' pop'; 
            if(pairTR) pairTR.className += (shouldRegen) ? ' fire-pop' : ' pop';
            
            setTimeout(() => {
                if(!shouldRegen) {
                    // Manual Clear (no regrowth)
                    t.style.visibility = 'hidden'; 
                    t.dataset.color = 'none'; t.dataset.pair = ""; // Mark as inactive
                    ...
                    // Trigger final screen if all clear
                    if(document.querySelectorAll('#en-grid .tile:not([style*="visibility: hidden"])').length === 0) triggerFinal(false);
                } else {
                    // Match-3 Bonus Regeneration
                    const pool = MASTER_DICT[GAME_STATE.unit].words;
